				...
				<form method='get' action='${pageContext.request.contextPath}/player/player_list.do' style="width: 300px;">
					<div class="input-group">
						<input type="text" name='keyword' class="form-control" 
							placeholder="도서 검색" value="${keyword}" />
						<span class="input-group-btn">
							<button class="btn btn-success" type="submit">
								<i class='glyphicon glyphicon-search'></i>
							</button>
							<a href="${pageContext.request.contextPath}/book/add_book.do" class="btn btn-primary">도서 추가</a>
						</span>
					</div>
				</form>
			</div>
		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="info text-center">저자</th>
		    	...
		    	<c:choose>
		    		<c:when test="${fn:length(bookList) > 0}">
		    			<c:forEach var="item" items="${bookList}">
		    				<tr>
					            <td class="text-center">${item.idBook}</td>
					            ...
					            <td class="text-center">${item.priceBook}</td>
					            <td class="text-center">${item.regDateBook}</td>
				        	</tr>
		    			</c:forEach>
		    		</c:when>
		    		<c:otherwise>
