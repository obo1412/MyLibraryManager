						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="info text-center">고유코드(로컬)</th>
									<th class="info text-center">도서식별번호</th>
									<th class="info text-center">도서관번호</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
									<c:when test="${fn:length(bookHeldList) > 0}">
										<c:forEach var="item" items="${bookHeldList}">
											<tr>
												...
												<td class="text-center">${item.localIdCode}</td>
												<td class="text-center">${item.bookIdBook}</td>
												<td class="text-center">${item.libraryIdLib}</td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
